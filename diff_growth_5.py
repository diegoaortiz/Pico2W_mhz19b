...
while True:
    ...
    for r in range(num_rings):
        ...
        # Le facteur d'activation monte ou descend lentement (Latence)
        # Ceci remplace l'ancien if r < active_rings
        ring_activation_factor[r] += (target_activation - ring_activation_factor[r]) * 0.15 
        
        current_activation = ring_activation_factor[r]
        
        # S'il est pratiquement inactif, passer au suivant
        ...
        # 3. CALCUL DE L'OPACITÉ ET DU RAYON
        
        base_radius = text_radius + r * RING_SPACING
        radius = base_radius + math.sin(t + ring_phases[r]) * 5 
        
        # Opacité Inversée (Effacement Centre) avec alpha_base plus élevé
        fade_outward_factor = r / (num_rings - 1)
        alpha_base = 1.0 # Base augmentée pour meilleure visibilité
        
        if target_active_rings > num_rings / 2: 
            # Opacité réduite moins agressivement sur les anneaux intérieurs
            alpha_base *= (0.6 + 0.4 * fade_outward_factor) 
        else:
            alpha_base *= (0.6 + 0.4 * (1 - fade_outward_factor))
            
        if alpha_base < 0.15: alpha_base = 0.15 # Minimum visible
        ...
        for i in range(segments): 
            ...
            # Disparition (alpha) - Utilisation du facteur d'activation
            target_alpha = alpha_base * current_activation
            
            # L'anneau s'efface en même temps qu'il devient inactif et plus transparent
