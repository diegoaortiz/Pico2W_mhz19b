...
while True:
    ...
    for r in range(num_rings):
        ...
        # ACCÉLÉRATION : Lissage de l'activation
        ...
        for i in range(segments): 
            ...
            # ACCÉLÉRATION : Transition de longueur
            ring_lengths[r][i] += (target_len - ring_lengths[r][i]) * SMOOTHING_FACTOR
            
            # ACCÉLÉRATION : Disparition (alpha)
            target_alpha = alpha_base * current_activation
            ring_alphas[r][i] += (target_alpha - ring_alphas[r][i]) * SMOOTHING_FACTOR 
